...
/// Given a sequence of Result values, return list of Ok values.
let oks l =
    (Seq.empty, l)
    ||> Seq.fold (fun acc v ->
        match v with
...
/// Given a sequence of Result values, return list of Error values.
let errors l =
    (Seq.empty, l)
    ||> Seq.fold (fun acc v ->
        match v with
...
/// Merges two FailureMaps by concatenating failure lists for matching keys.
...
/// Maps the keys of a Map using the provided function.
let mapKeys (fn: 'K -> 'L) (m: Map<'K, 'T>) : Map<'L, 'T> =
    Map.toSeq m |> Seq.map (fun (k, v) -> (fn k, v)) |> Map.ofSeq

/// Tests if two values are equal and returns an option with the provided failure if not.
