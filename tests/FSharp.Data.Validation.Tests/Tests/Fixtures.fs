...
type UserIdFailures = | LessThanOneFailure
...
type EmailAddressFailures = | InvalidEmail
...
let ``UserContactDTO: Returns single failure when email is invalid`` (PositiveInt uid) =
    ...
    let expected =
        Invalid([], Map.ofList [ ([ mkName "EmailAddress" |> Option.get ], [ EmailAddressFailure InvalidEmail ]) ])
...
[<Property>]
let ``UserContactDTO: Returns multiple failures when email is invalid and contact preference is phone``
    (PositiveInt uid)
    =
    let phone = None
    let email = "test@test"
