...
[<Property>]
let ``map: Does not change the contents of an invalid proof`` (gf1, NonWhiteSpaceString n1, lf1) =
...
[<Property>]
let ``map: Converts a Proof<f, a> to a Proof<f, b>`` (n: int) =
    let input = Valid n
    let result = Proof.map (fun a -> a.ToString()) input
    Assert.Equal(Valid(n.ToString()), result)

[<Property>]
let ``mapInvalid: Does not change the contents of a valid proof`` (n: int) =
    let input = Valid n
    let result = Proof.mapInvalid (fun a -> a + 1) input
    Assert.Equal(input, result)

[<Property>]
let ``mapInvalid: Converts a Proof<f, a> to a Proof<g, a>`` (gf1: int, lf1: int, NonWhiteSpaceString n1) =
    let field1 = mkName n1 |> Option.get
    let input: Proof<int, int> = Invalid([ gf1 ], Map.ofList [ ([ field1 ], [ lf1 ]) ])
    let result = Proof.mapInvalid (fun a -> a.ToString()) input

    let expected: Proof<string, int> =
        Invalid([ gf1.ToString() ], Map.ofList [ ([ field1 ], [ lf1.ToString() ]) ])
...
let ``combine: one valid and one invalid proof results in invalid proof``
    ...
    let input1 = Valid a
    let input2 = Invalid([ b ], Map.ofList [ ([ field1 ], [ d ]) ])
    let result = Proof.combine (+) input1 input2
...
let ``combine: one invalid and one valid proof results in invalid proof``
    ...
    let input1 = Invalid([ a ], Map.ofList [ ([ field1 ], [ c ]) ])
    let input2 = Valid d
    let result = Proof.combine (+) input1 input2
    Assert.Equal(input1, result)

[<Property>]
let ``combine: two invalid proofs results in concatenated errors`` (gf1, gf2, lf1: string, lf2: string, lf3: string) =
    let field1 = mkName "Field1" |> Option.get
    let field2 = mkName "Field2" |> Option.get
    let input1 = Invalid([ gf1 ], Map.ofList [ ([ field1 ], [ lf1 ]) ])
...
let ``serialize: valid proof of type T should result in JSON representing T`` () =
    ...
    //Assert
    json |> should equal "{\"MyName\":\"John Smith\",\"MyInt\":42}"
...
let ``serialize: invalid proof of type T should result in JSON representing the failures`` () =
    ...
    let lfs =
        Map.ofList [ ([ myName ], [ EmptyName ]); ([ myObj; myInt ], [ IntToSmall ]) ]
    ...
    |> should
        equal
        "{\"failures\":[\"John Smith\\u0027s number can only be 42, not 41.\"],\"fields\":{\"myName\":[\"MyName cannot be empty.\"],\"myObj.myInt\":[\"MyInt cannot be less than 42.\"]}}"
