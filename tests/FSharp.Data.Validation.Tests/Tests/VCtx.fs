...
[<Property>]
let ``map: Transforms a ValidCtx`` (a: int) =
    let input = ValidCtx a
    let result = VCtx.map (fun b -> b.ToString()) input
    Assert.Equal(ValidCtx(a.ToString()), result)

[<Property>]
let ``map: Transforms a DisputedCtx while preserving failures`` (a: int, NonWhiteSpaceString n1, lf1: int, gf1: int) =
...
[<Property>]
let ``map: Makes no changes to a RefutedCtx`` (NonWhiteSpaceString n1, lf1: int, gf1: int) =
    let field1 = mkName n1 |> Option.get
    // Todo: make failures of arbitrary length
    let gfs = [ gf1 ]
    let lfs = Map.ofList [ ([ field1 ], [ lf1 ]) ]
...
[<Property>]
let ``bind: Transforms a ValidCtx`` (a: int) =
    let input = ValidCtx a
    let result = VCtx.bind (fun b -> ValidCtx(b.ToString())) input
    Assert.Equal(ValidCtx(a.ToString()), result)

[<Property>]
let ``bind: Makes no changes to a RefutedCtx`` (NonWhiteSpaceString n1, lf1: int, gf1: int) =
    let field1 = mkName n1 |> Option.get
    // Todo: make failures of arbitrary length
    let gfs = [ gf1 ]
    let lfs = Map.ofList [ ([ field1 ], [ lf1 ]) ]
...
[<Property>]
let ``mergeSources with two ValidCtx should return ValidCtx with tuple`` (a: int, b: int) =
...
[<Property>]
let ``VCtxBuilder.Bind: Transforms a ValidCtx`` (a: int) =
    let input = ValidCtx a
    Assert.Equal(ValidCtx(a.ToString()), VCtxBuilder().Bind(input, (fun b -> ValidCtx(b.ToString()))))

[<Property>]
let ``VCtxBuilder.Bind: Makes no changes to a RefutedCtx`` (NonWhiteSpaceString n1, lf1: int, gf1: int) =
    let field1 = mkName n1 |> Option.get
    let gfs = [ gf1 ]
    let lfs = Map.ofList [ ([ field1 ], [ lf1 ]) ]
    let input = RefutedCtx(gfs, lfs)
...
[<Property>]
let ``VCtxBuilder.MergeSources: Merges two ValidCtx into a tuple`` (a: int, b: int) =
    let input = ValidCtx a, ValidCtx b
    Assert.Equal(ValidCtx(a, b), VCtxBuilder().MergeSources(input))
...
let ``VCtxBuilder.MergeSources: Merging two DisputedCtx results in DisputedCtx``
    ...
    let expected1 =
        DisputedCtx(gfs1 @ gfs2, Map.ofList [ ([ field1 ], [ lf1; lf2 ]) ], (a, b))

    let expected2 =
        DisputedCtx(gfs2 @ gfs1, Map.ofList [ ([ field1 ], [ lf2; lf1 ]) ], (b, a))
...
[<Property>]
let ``VCtxBuilder.Optional: Optional of a RefutedCtx returns RefutedCtx`` (NonWhiteSpaceString n1, lf1: int, gf1: int) =
    let field1 = mkName n1 |> Option.get
    let gfs1 = [ gf1 ]
    let lfs1 = Map.ofList [ ([ field1 ], [ lf1 ]) ]
...
[<Property>]
let ``VCtxBuilder.Optional: Optional of a ValidCtx with None returns ValidCtx`` (NonWhiteSpaceString n1) =
    let field1 = mkName n1 |> Option.get
    let input = ValidCtx(Field(field1, None))
    Assert.Equal(input, VCtxBuilder().Optional(input, mk5))

[<Property>]
let ``VCtxBuilder.Optional: Optional of a ValidCtx with Some valid returns ValidCtx`` (NonWhiteSpaceString n1) =
    let field1 = mkName n1 |> Option.get
...
[<Property>]
let ``VCtxBuilder.Optional: Optional of a ValidCtx with Some disputed returns DisputedCtx`` (NonWhiteSpaceString n1) =
    let field1 = mkName n1 |> Option.get
...
[<Property>]
let ``VCtxBuilder.Optional: Optional of a ValidCtx with Some refuted returns RefutedCtx`` (NonWhiteSpaceString n1) =
    let field1 = mkName n1 |> Option.get
    let input = ValidCtx(Field(field1, Some 1))
...
[<Property>]
let ``VCtxBuilder.Optional: Optional of a DisputedCtx with None returns DisputedCtx`` (lf1: int, gf1: int) =
    let field1 = mkName "Field1" |> Option.get
    let field2 = mkName "Field2" |> Option.get

    let input =
        DisputedCtx([ gf1 ], Map.ofList [ ([ field1 ], [ lf1 ]) ], Field(field2, None))

    Assert.Equal(input, VCtxBuilder().Optional(input, mk5))

[<Property>]
let ``VCtxBuilder.Optional: Optional of a DisputedCtx with Some valid returns DisputedCtx`` (lf1: int, gf1: int) =
    let field1 = mkName "Field1" |> Option.get
    let field2 = mkName "Field2" |> Option.get

    let input =
        DisputedCtx([ gf1 ], Map.ofList [ ([ field1 ], [ lf1 ]) ], Field(field2, Some 5))

    let expected =
        DisputedCtx([ gf1 ], Map.ofList [ ([ field1 ], [ lf1 ]) ], Global(Some 5))

    Assert.Equal(expected, VCtxBuilder().Optional(input, mk5))

[<Property>]
let ``VCtxBuilder.Optional: Optional of a DisputedCtx with Some disputed returns DisputedCtx`` (lf1: int, gf1: int) =
    let field1 = mkName "Field1" |> Option.get
    let field2 = mkName "Field2" |> Option.get

    let input =
        DisputedCtx([ gf1 ], Map.ofList [ ([ field1 ], [ lf1 ]) ], Field(field2, Some 1))

    let expected =
        DisputedCtx([ gf1 ], Map.ofList [ ([ field1 ], [ lf1 ]); ([ field2 ], [ -5 ]) ], Global(Some 1))

    Assert.Equal(expected, VCtxBuilder().Optional(input, mk5))

[<Property>]
let ``VCtxBuilder.Optional: Optional of a DisputedCtx with Some refuted returns RefutedCtx`` (lf1: int, gf1: int) =
    let field1 = mkName "Field1" |> Option.get
    let field2 = mkName "Field2" |> Option.get
    ...
    let expected =
        RefutedCtx([ gf1 ], Map.ofList [ ([ field1 ], [ lf1 ]); ([ field2 ], [ -5 ]) ])
...
[<Property>]
let ``VCtxBuilder.DisputeWithFact: When validation fails, the valid context becomes a disputed context``
    (NegativeInt i)
    =
    let a = Global i
    let ctx = ValidCtx a
...
[<Property>]
let ``VCtxBuilder.DisputeWithFact: When validation fails, the failure is added to the disputed context``
    (NegativeInt i)
    =
    let a = Global i
    let failure1 = "failure1"
...
let ``VCtxBuilder.RefuteWith: When validation fails, the valid context becomes a refuted context`` (NegativeInt i) =
    ...
    let func x =
        if x > 0 then Ok success else Error failure
...
let ``VCtxBuilder.RefuteWith: When validation fails, the disputed context becomes a refuted context`` (NegativeInt i) =
    ...
    let func x =
        if x > 0 then Ok success else Error failure2
...
let ``VCtxBuilder.RefuteWithProof: When element validation fails, the valid context becomes a refuted context``
    ...
    let func x =
        if x > 0 then
            ...
            Invalid([ failure ], Map.ofList [ ([ field1 ], [ failure ]) ])

    let result = VCtxBuilder().RefuteWithProof(ctx, func)
    let name = mkName "[1]" |> Option.get

    let expected =
        RefutedCtx([], Map.ofList [ ([ field1 ], [ failure ]); ([ name ], [ failure ]) ])

    Assert.Equal(expected, result)

[<Property>]
let ``VCtxBuilder.RefuteWithProof: When element validation fails, the disputed context becomes a refuted context``
    (NegativeInt i)
    =
    let field1 = mkName "field1" |> Option.get
    let field2 = mkName "field2" |> Option.get
    ...
    let func x =
        if x > 0 then
            ...
            Invalid([ failure ], Map.ofList [ ([ field2 ], [ failure ]) ])
...
[<Property>]
let ``VCtxBuilder.RefuteWithProof: When element validation succeeds, the valid context remains the same``
    (PositiveInt i)
    =
    let field1 = mkName "field1" |> Option.get
    let a = Element(1, i)
...
[<Property>]
let ``VCtxBuilder.RefuteWithProof: When element validation succeeds, the disputed context remains the same``
    (PositiveInt i)
    =
    let field1 = mkName "field1" |> Option.get
    let field2 = mkName "field2" |> Option.get
...
[<Property>]
let ``VCtxBuilder.RefuteWithProof: When field validation fails, the valid context becomes a refuted context``
    (NegativeInt i)
    =
    let field1 = mkName "field1" |> Option.get
    let field2 = mkName "field2" |> Option.get
    ...
    let expected =
        RefutedCtx([], Map.ofList [ ([ field1 ], [ failure ]); ([ field2 ], [ failure ]) ])

    Assert.Equal(expected, result)

[<Property>]
let ``VCtxBuilder.RefuteWithProof: When field validation fails, the disputed context becomes a refuted context``
    (NegativeInt i)
    =
    let field1 = mkName "field1" |> Option.get
    let field2 = mkName "field2" |> Option.get
    ...
    let func x =
        if x > 0 then
            ...
            Invalid([ failure ], Map.ofList [ ([ field3 ], [ failure ]) ])
...
[<Property>]
let ``VCtxBuilder.RefuteWithProof: When field validation succeeds, the valid context remains the same``
    (PositiveInt i)
    =
    let field1 = mkName "field1" |> Option.get
    let field2 = mkName "field2" |> Option.get
...
[<Property>]
let ``VCtxBuilder.RefuteWithProof: When field validation succeeds, the disputed context remains the same``
    (PositiveInt i)
    =
    let field1 = mkName "field1" |> Option.get
    let field2 = mkName "field2" |> Option.get
...
[<Property>]
let ``VCtxBuilder.RefuteWithProof: When gobal validation fails, the valid context becomes a refuted context``
    (NegativeInt i)
    =
    let a = Global i
    let ctx = ValidCtx a
...
[<Property>]
let ``VCtxBuilder.RefuteWithProof: When global validation fails, the disputed context becomes a refuted context``
    (NegativeInt i)
    =
    let a = Global i
    let failure1 = "failure1"
...
[<Property>]
let ``VCtxBuilder.RefuteWithProof: When global validation succeeds, the valid context remains the same``
    (PositiveInt i)
    =
    let a = Global i
    let ctx = ValidCtx a
...
[<Property>]
let ``VCtxBuilder.RefuteWithProof: When global validation succeeds, the disputed context remains the same``
    (PositiveInt i)
    =
    let a = Global i
    let failure1 = "failure1"
...
[<Fact>]
let ``VCtxBuilder.RefuteWithValidation: Maps failures and refutes on invalid proof`` () =
    let field = mkName "field" |> Option.get
    let ctx = ValidCtx(Field(field, "bad"))
    let fn _ = Invalid([ 1 ], Map.empty)
    let mapFailure (i: int) = sprintf "E%i" i
    let result = VCtxBuilder().RefuteWithValidation(ctx, fn, mapFailure)
    let expected = RefutedCtx([], Map.ofList [ ([ field ], [ "E1" ]) ])
    Assert.Equal(expected, result)

[<Fact>]
let ``VCtxBuilder.RefuteWithValidation: Continues on valid proof`` () =
    let field = mkName "field" |> Option.get
    let ctx = ValidCtx(Field(field, 5))
    let fn x = Valid(x + 1)
    let mapFailure (_: string) = "mapped"
    let result = VCtxBuilder().RefuteWithValidation(ctx, fn, mapFailure)
    let expected = ValidCtx(Field(field, 6))
    Assert.Equal(expected, result)

[<Fact>]
let ``VCtxBuilder.RefuteEachWithValidation: Refutes when any element proof is invalid`` () =
    let field = mkName "items" |> Option.get
    let items = [ 1; 2; 3 ]
    let ctx = ValidCtx(Field(field, items :> seq<int>))

    let fn x =
        if x % 2 = 0 then
            Invalid([ 10 ], Map.empty)
        else
            Valid(x * 2)

    let mapFailure (i: int) = sprintf "E%i" i
    let result = VCtxBuilder().RefuteEachWithValidation(ctx, fn, mapFailure)

    match result with
    | RefutedCtx(gfs, lfs) ->
        Assert.Equal<string list>([], gfs)
        Assert.True(Map.count lfs > 0)
    | _ -> failwith "Expected RefutedCtx"
